...
var divTracker = $('.cell').bind('click', function(){
    var tracker = $(this).attr('id');
    console.log(tracker + "from tracker")
    return tracker;// gets the id of a clicked link that has a class of menu. THIS NUMBER IS NOT PULLING THROUGH TO game.
});
...
var gameLogic = {
  ...
  altClicks: $(function(){
    ...
    if (hits % 2 !== 0) { //for hits 2,4,6,8 etc
      ...
      $(this).append( $imgX );
      var x = divTracker;// need to pull through the DIV ID
      ...
      console.log(gameLogic.game);
        ...
        $(this).append( $imgO);
        var y = divTracker;// need to pull through the DIV ID from divTracker above
